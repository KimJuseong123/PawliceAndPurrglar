...
| 효과음 37개 | freesound.org | CC0 (공유 저작물) | 가능, 표기 의무 없음 |
...
**37개 전부 CC0**로, freesound.org에서 CC0 필터로만 받았다. CC0는 표기 의무가 없다.

세 번에 나눠 들어왔고 셋 다 같은 경로다 — 최초 10종, 2026-08-09의 11종(C-4~C-11),
같은 날의 16종(C-1~C-3).

그중 `sfx_firework_bang.wav` 하나는 **받은 파일이 아니라 만든 파일이다.** CC0 녹음
두 개를 이어 붙인 것이므로 결과물도 CC0이고, 원본 두 개는
`Assets/_Project/ArtSource/Audio/`에 있다.
