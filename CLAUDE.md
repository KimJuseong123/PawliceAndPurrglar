...
> **실내 좌표를 도시 좌표처럼 쓰지 않는다.** 실내는 별도 씬이 아니라 **같은 씬 안에서
> 멀리 떨어뜨려 놓은 방**이다. 그래서 보석상 진열대의 좌표는 거리에서 걸어갈 수 없는
> 자리이고, 그 자리를 가리킨 경찰 화면의 경보 신호는 **맵 아래쪽 빈 들판**을 향했다
> (`ISSUE-073`). **아무것도 로그를 남기지 않는다** — 경보는 울리고 신호는 10초를 채우고
> 화살표는 거리에 따라 크기가 변한다. 전부 동작하면서 아무 데도 아닌 곳을 가리킨다.
> 실내에서 나온 위치는 `HouseInterior.ExitPosition`으로 번역한다. 검사는 **밖에서 울린
> 경보가 그대로 지나가는지도** 본다 — 방 안 사례만 보면 모든 경보를 옮기는 코드도 통과한다.
>
> 그리고 방의 범위는 **방의 로컬 공간에서** 잰다. 월드 축으로 재면 돌려 놓은 방은
> 가로세로가 바뀐 채로 검사된다.

> **효과음 음량을 귀로 고르지 않는다.** `Report Sound Levels`가 뱅크의 클립을 전부
> 디코드해 RMS를 찍는다. **피크는 쓸모가 없다** — 40개 중 대부분이 1.0이고(노멀라이즈가
> 그렇게 만든다), 피크 0.37짜리가 가장 시끄러운 축에 있었다. 그리고 음량은 에셋이 아니라
> `SoundBankSetup.Volumes`에 이유와 함께 적는다. 에셋에만 있는 숫자는 근거가 없고 병합에서
> 가장 먼저 사라진다.
>
> 같은 파일을 두 id가 공유하면 **음량도 함께 정한다.** 한 녹음이 호출 경로에 따라 두
> 음량으로 나는 믹스는 설명할 수 없다.
...
Report Sound Levels               뱅크 클립 40개의 RMS 실측 (음량은 이걸 보고 정한다)
...
승리에 체포가 3회 필요하고 그 사이에 감옥 10초가 있으므로 `full`은 **60초**가
