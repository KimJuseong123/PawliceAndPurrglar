...
2026-07-26 승인된 범위 예외로 단계 A 완료 전에도 다음 수직 슬라이스만
본게임에 연결한다.

- `V`를 누르는 동안 최대 5초 녹음
- OpenAI STT를 통한 한국어 텍스트 변환
- LLM Structured Output을 통한 `TRACK`, `DISTRACT`, `NONE` 분류
- 경찰 강아지 `TRACK`과 도둑 고양이 `DISTRACT` 실행
- 음성 실패 시 숫자키 입력 유지
- 로컬 중계 서버에서만 OpenAI API 키 보관

이 예외는 전체 8개 명령, 상시 듣기, 자유 대화, 런타임 생성형 행동 판단을
허용하지 않는다. LLM은 제한 명령 ID만 선택하고 실제 실행 가능 여부와 행동은
결정적인 게임 규칙 코드가 소유한다.
