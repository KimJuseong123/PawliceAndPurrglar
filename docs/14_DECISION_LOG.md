...
### DEC-UI-002: 아이템 설명은 캔버스에 하나뿐인 툴팁이 맡고, 문구는 아이템 쪽에 둔다 (2026-08-10)

- Context: 인벤토리와 퀵슬롯의 아이콘만으로는 그 물건이 무엇을 하는지 알 수 없다.
  소품 아홉 종은 효과가 서로 다르고 보물 30여 종은 운반 비용이 다른데, 칸에는
  그림과 개수만 있다.
- Decision: 캔버스마다 **툴팁을 하나** 두고 모든 칸이 공유한다. 칸에는
  `ItemSlotTooltipTrigger`만 붙고, 무엇을 말할지는 칸이 바인딩될 때 함께 받은
  `ItemTooltipContent`가 들고 있다.
- Alternative rejected: **칸마다 숨긴 패널을 두는 안.** 서른 개의 패널은 서른 개의
  레이아웃이고, 그중 하나라도 레이캐스트를 받으면 **자기가 설명하던 칸의
  `PointerExit`를 가로챈다** — 그러면 마우스가 움직이는 속도로 깜빡인다. 같은 이유로
  툴팁 안의 모든 그래픽은 `raycastTarget = false`다.
- Decision: **문구는 아이템이 정의된 곳에서 온다.** 소품은 `ThrowableCatalog`가
  지속시간 상수 옆에서 문장을 만들고, 보물은 `LootDefinition`의 등급·운반 종류·경보
  여부에서 파생한다. `ItemTooltipCatalog`는 둘을 문장으로 바꾸기만 한다.
- Alternative rejected: **보물 에셋에 설명 필드를 새로 만드는 안.** 문서가 권한
  방식이지만 정의가 30여 개이고 전부 `LootDataSetup`이 생성한다 — 필드를 만들면
  30개를 작성해야 하고, 그중 하나가 비어 있으면 **검증기는 통과하고 화면만 빈다.**
  등급과 운반 종류가 이미 도둑이 알아야 할 두 가지를 말한다.
- Decision: 분류는 `GetOwner`와 `GetUse`를 **조합해서** 만든다
  (`경찰 전용 · 설치 아이템`). 손으로 아홉 줄을 적으면 픽업 지점이 강제하는 소유자와
  툴팁이 말하는 소유자가 갈라질 수 있다.
- Constraint: 문장은 **정적 문자열로 캐시한다.** HUD는 25칸을 매 프레임 다시
  바인딩하므로, 호출할 때마다 문자열을 만들면 아무도 보지 않는 패널을 위해 프레임마다
  할당이 생긴다.
- Constraint: 화면 밖으로 나가면 **뒤집고 나서 clamp한다.** 순서가 중요하다 —
  뒤집기만으로는 캔버스보다 큰 패널을 담지 못하고, clamp만으로는 패널이 커서 밑으로
  밀려 들어간다.
- Consequence: **`Resources/HudCanvas.prefab`을 재생성해야 적용된다.** 게임의 HUD는
  씬이 아니라 그 프리팹을 인스턴스화한다. 코드만 고치면 툴팁 없는 낡은 프리팹이 계속
  뜨고, 에디터에서 `BuildRuntimeCanvas`를 부르는 테스트는 통과한다.
- Consequence: 너구리 창의 칸도 같은 빌더를 지나가므로 툴팁을 함께 얻었다. 문서의
  적용 범위(가방·퀵슬롯·고양이 가방) 밖이지만, 같은 칸을 두 가지로 만들 이유가 없다.
- Still open: **실제 마우스로 호버해 본 것은 아니다.** 위치·clamp·가독성·레이캐스트는
  Play Mode 8건이 재고, 눈으로 본 검증은 없다.
