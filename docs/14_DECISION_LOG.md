...
### DEC-027 OpenAI 음성 분류를 핵심 2개 명령에 한정해 먼저 연결한다

- 날짜: 2026-07-26
- 상태: 확정
- 배경: Windows 내장 받아쓰기는 현재 개발 PC에서 실패했지만 사용자가 실제 음성으로 역할별 동물을 조종하는 기능을 요청했다.
- 고려한 선택지: 전체 음성 에픽 보류, 로컬 오프라인 모델, Unity 직접 OpenAI 호출, 로컬 보안 중계 서버를 통한 제한 명령 분류
- 결정: `V` Push-to-Talk, `gpt-4o-mini-transcribe` 전사와 `gpt-5.6-luna` Structured Output으로 경찰 `TRACK`, 도둑 `DISTRACT`, 실패 `NONE`만 분류한다.
- 이유: 실제 음성 차별점을 검증하면서 LLM의 출력 범위와 게임 영향, API 키 노출을 최소화할 수 있다.
- 장점: 한국어 외부 STT 사용, 역할별 결정적 명령 실행, 숫자키와 동일한 검증 경로, 키보드 실패 복구
- 단점: 로컬 중계 서버와 네트워크가 필요하고 전체 8개 명령은 제공하지 않는다.
- 영향: Companions, Input, Voice, UI, Integration, Config, Tests, Submission
- 재검토 조건: 핵심 2개 명령의 지연·정확도 플레이테스트가 통과하거나 제출 빌드용 원격 중계 서버를 선택할 때
