...
그레이박스 마을에서 도둑의 보물 획득·운반·드롭·판매와 전용 HUD까지 이어지는 핵심 루프를 검증한다.
...
- 작업: 도둑 HUD
- 상태: `DONE`
- 결과: 읽기 전용 도둑 HUD, 씬 연결, 자동 테스트와 WindowsPlayer 화면 검증 완료

## 바로 다음 작업

1. `MERCHANT-001`: 너구리 상인 판매 범위
2. `ARREST-001`: 체포 범위 감지
...
| 2026-07-25 | UI-003 Edit Mode 테스트 | 65/65 통과 |
