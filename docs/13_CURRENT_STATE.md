...
| `localRole` | Police | **Unassigned** | `ISSUE-016` |

**NET-004는 완료다.** 두 화면의 남은 시간이 0.02초 이내로 일치하고,
클라이언트는 자기 타이머를 돌리지 않으며(`matchRemoteControlled: true`)
승패도 호스트만 결정한다.

**NET-003은 절반이다.** 링크가 양쪽에 스폰되고 좌표가 정확히 복제되며,
클라이언트의 모터와 `CharacterController`가 꺼져 호스트만 시뮬레이션한다.
그러나 클라이언트가 경기 씬에서 자기 역할을 읽지 못해 입력을 보내지 않는다
(`ISSUE-016`). 즉 **클라이언트 조작이 아직 동작하지 않는다.**
