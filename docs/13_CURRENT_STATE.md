...
- 결과: **빌드된 로비의 버튼 6개가 전부 죽어 있어 사람이 하는 멀티플레이가
  ...
  맵을 80×68m로 확장하고 집 12채를 넣었으며, 카메라를 15% 당겨 화면상 약 17%
  크게 보이게 했다. Edit Mode 139개, Play Mode 77개 통과.
...
1. `ISSUE-011`: 도둑 판매 승리 경로 확보. `NET-010`의 남은 두 시나리오
   (도둑 승리, 판매·체포 동시)가 여기에 막혀 있다
2. `MODEL-002` 애니메이션 클립 6종 (작업자 몫, 현재 클립 0개)
3. `MAP-004` 루트 밸런스, `MAP-005` 스폰 확정
4. 관문 B 플레이테스트 (2인 실기)

## 차단 요소

- 도둑 판매 승리 경로 부재로 관문 B가 `BLOCKED` (`ISSUE-011`).
  `NET-010`의 도둑 승리·동시 발생 시나리오도 같은 이유로 미측정
