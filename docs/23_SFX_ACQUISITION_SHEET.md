...
### 길이가 슬롯보다 긴 것들 — 심한 둘은 잘랐다

| ID | 클립 | 게임에서 쓰는 시간 | 상태 |
|---|---:|---:|---|
| `CaseKeyUnlock` | 14.23 → **2.80초** | 2.8초 | **잘랐다.** 8.50~11.30초 구간 |
| `NoisePropFuse` | 8.10 → **2.50초** | 2.5초 | **잘랐다.** 0~2.50초 |
| `SensorTripped` | 3.40초 | 2.5초 | 0.9초 남는다 |
| `TrapPlaced` | 3.08초 | — | |
| `ThrowCharge` | 2.59초 | 1.0초 | 1.6초 남는다 |
| `Stunned` | 2.56초 | 1.2초 | 1.4초 남는다 |
| `LureTaken` | 5.00초 | 4.0초 | 1.0초 남는다 |

`NoisePropBang`(7.06초)과 `NoiseHeardFar`(5.00초)는 폭발이라 긴 것이 이상하지 않다.

> **자를 곳은 길이가 아니라 파형을 보고 정한다.** `sfx_case_unlock`은 **2초 가까이
> 무음으로 시작하고** 자물쇠 소리는 8.5초 뒤에 있었다. 앞 2.8초를 가져왔으면 거의 무음인
> 클립이 나왔을 것이고 **뱅크에는 정상으로 들어가고 로그도 깨끗했을 것이다.** 0.1초
> 단위 RMS로 에너지가 가장 큰 2.8초 창을 골랐다(전체의 61%). 도화선은 처음부터 끝까지
> 같은 지글거림이라 앞이 곧 도화선이다.
>
> 시작 5~10ms 페이드인, 끝 25ms 페이드아웃. 그냥 끊으면 딸깍 소리가 난다.
> **같은 이름으로 덮어쓴다** — 새 이름은 뱅크가 옛 파일을 계속 가리키게 만든다.
