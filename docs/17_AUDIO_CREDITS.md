...
| 파일 | 전 | 후 | 가져온 구간 | 이유 |
|---|---:|---:|---|---|
| `sfx_case_unlock` | 14.23초 | **2.80초** | **8.50~11.30초** | 여는 데 2.8초 (`LootDisplayCase.UnlockSeconds`) |
| `sfx_fuse_burn` | 8.10초 | **2.50초** | 0.00~2.50초 | 도화선이 2.5초 (`ThrowableCatalog.FireworkFuseSeconds`) |

**`sfx_case_unlock`은 앞에서 자르면 안 됐다.** 파일이 **2초 가까이 무음으로 시작하고**
실제 자물쇠 소리는 8.5초 뒤에 몰려 있다 — 앞 2.8초를 가져왔으면 거의 무음인 클립이
나왔을 것이고, **뱅크에는 정상으로 들어가고 로그도 깨끗했을 것이다.** 0.1초 단위로 RMS를
재서 2.8초 창 중 에너지가 가장 큰 곳을 골랐다(전체의 61%). 길이만 맞추고 내용을 안 보면
이런 것을 놓친다.

도화선은 반대다. 처음부터 끝까지 같은 지글거림이라 **앞이 곧 도화선**이고, 2.5초에서
끝나므로 폭죽이 터지는 순간 소리가 멎는다.

둘 다 시작에 5~10ms 페이드인, 끝에 25ms 페이드아웃을 넣었다. 파형을 그냥 끊으면 딸깍
소리가 나고, 그건 듣는 사람이 매번 알아채는 유일한 잡음이다. 끝 표본은 정확히 0이다.

**같은 이름으로 덮어썼다.** 새 이름으로 넣으면 뱅크가 옛 파일을 계속 가리킨다. 자르기
전 원본이 필요하면 `git show ade15f1:<경로>`로 꺼낸다.
