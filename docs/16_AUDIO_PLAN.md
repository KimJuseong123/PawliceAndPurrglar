...
> **받으러 갈 때는 [23_SFX_ACQUISITION_SHEET.md](23_SFX_ACQUISITION_SHEET.md)를
> 연다.** 아래 목록에 **파일 이름과 freesound 검색어**를 채운 작업표이고, 못 찾은
> 9종의 대체 검색어와 재사용 후보도 거기 있다. 이 절은 "무엇이 필요한가", 그쪽은
> "어떻게 구해서 어떤 이름으로 넣는가"다.
