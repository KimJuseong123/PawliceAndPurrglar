...
| UI-023 | P1 | DONE | **아이템 설명 Hover 툴팁** (`ItemTooltipView`). 칸에 커서를 0.2초 올리면 아이콘·이름·분류·설명·사용법이 뜬다. 캔버스에 **하나**를 두고 가방 25칸·퀵슬롯 4칸·고양이 가방·너구리 창이 공유한다. 화면 밖으로 나가면 커서 반대쪽으로 뒤집고 마지막에 clamp한다. 문구는 아이템 쪽(`ThrowableCatalog`·`LootDefinition`)에서 오고 슬롯 코드에는 이름 분기가 없다 |
