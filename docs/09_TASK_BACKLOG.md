...
| NET-003 | P0 | IN_PROGRESS | 이동 동기화 — 복제는 동작, 클라이언트 조작이 `ISSUE-016`으로 막힘 |
| NET-004 | P0 | DONE | 경기 상태와 타이머 동기화 (양쪽 233.05초 일치 실측) |
| NET-005 | P0 | READY | 보물 소유권 동기화 |
| NET-006 | P0 | READY | 판매와 점수 동기화 |
| NET-007 | P0 | READY | 체포 판정 동기화 |
| NET-008 | P1 | READY | 재경기 동기화 |
| NET-009 | P1 | READY | 연결 종료 처리 |
| NET-010 | P1 | READY | 네트워크 전체 회귀 테스트 |
...
`NET-003`~`010`은 `BLOCKED`에서 `READY`로 올렸다. 다만 착수 전에
`DEC-P01`(네트워크 권한 구조)을 확정해야 한다. 아래 `단계 7 착수 조건` 참고.
