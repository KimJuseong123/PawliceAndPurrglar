...
| LOOT-014 | P0 | TODO | **소품 다섯 종을 실내 랜덤 스폰으로 옮긴다.** 바나나·개껌·폭죽·고무닭·냉동문어는 지금 잔디밭에 2.2m 간격으로 늘어선 줄 하나가 **유일한 획득처**다(씬의 수색 컨테이너는 0개, 상점은 끈끈이·센서등·참치캔만 판다). 상점에 안이 없던 시절의 임시 배치다. 슈퍼마켓·1층집·2층집의 **보물이 쓰지 않은 자리**에 방당 1~2개만 뽑아 놓는다 — 3자리를 정확히 채우면 방이 항상 가득 차서 `자리 − 3`이 보장하던 "매판 다른 배치"가 죽는다. 배분: 슈퍼마켓 = 바나나·개껌·냉동문어(냉동식품), 1층집 = 바나나·개껌·고무닭, 2층집 = 폭죽·고무닭·바나나. 들어가면 `CreateShopShelfPickups`의 줄은 지운다 |
