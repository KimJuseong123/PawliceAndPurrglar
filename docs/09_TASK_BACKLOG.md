...
- 단계 A 프로토타입이 끝나기 전 전체 음성과 최종 모델 작업을 시작하지 않는다.
- `DEC-027`의 핵심 2개 음성 수직 슬라이스만 승인된 범위 예외로 진행한다.
...
핵심 프로토타입 완료 전 전체 명령은 구현하지 않는다. `VOICE-008`만
`DEC-027`에 따른 제한적 예외다.
...
| VOICE-008 | P1 | IN_PROGRESS | OpenAI 기반 `TRACK`·`DISTRACT` 음성 수직 슬라이스 |
