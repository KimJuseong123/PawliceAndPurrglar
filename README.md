...
프로토타입에서는 모든 명령을 숫자키 `1`~`4`로 실행합니다.
...
5. 강아지와 고양이 명령을 숫자키 `1`~`4`로 구현합니다.
...
- 네트워크 패키지의 최종 채택과 서버 구조
- Windows 이후 WebGL 지원 여부
- 명령별 정확한 효과와 쿨타임
- 보물 종류와 목표 금액
- 체포 방식과 세부 밸런스
- 최종 맵 크기와 건물 수
- 실제 음성 인식 및 자연어 분류 방식

세부 상태는 이후 `docs/13_CURRENT_STATE.md`에서 관리합니다.
...
2. 저장소를 클론합니다.
3. **TopDown Engine은 없어도 됩니다.** 이 외부 에셋은 라이선스가 재배포를
   금지하므로 저장소에 포함하지 않습니다. 보유하고 있다면 `Assets/TopDownEngine/`
   경로로 임포트하세요.

### TopDown Engine이 없을 때 (대부분의 경우)

프로젝트는 **컴파일되고 빌드되고 플레이됩니다.** 경기, 네트워크, 맵, 상호작용은
전부 정상입니다. 차이는 하나뿐입니다.

**경찰과 도둑의 걷기·달리기 애니메이션이 재생되지 않습니다.** 두 캐릭터는 선 자세를
유지한 채로 이동합니다. 동물(강아지·고양이·너구리)은 뼈를 직접 움직이는 절차적
방식이라 영향받지 않습니다.

이유는 `CharacterLocomotion.controller`가 참조하는 클립 6개가 TopDown Engine의
파일이라는 것입니다. 저장소에 그 파일을 넣을 수 없으므로 클립 참조가 끊깁니다.

이때 `AnimatorClipGuard`가 Animator를 끕니다. 끄지 않으면 휴머노이드 리타게팅이
캐릭터를 주저앉혀 **땅에 묻힌 것처럼 보입니다.** 실측값입니다.

| 상태 | 경찰 발 높이 | 경찰 힙 높이 |
|---|---|---|
| 가드 없음 (버그) | −0.14 m (지면 아래) | 0.07 m (주저앉음) |
| 가드 동작 | 0.20 m | 0.48 m |
| TopDown Engine 보유 | 0.13 m | 0.45 m |

가드가 동작하면 콘솔에 한 번 경고가 남습니다. 정상 동작이며 무시해도 됩니다.

최종 캐릭터 클립(`MODEL-002`)이 들어오면 이 의존성은 완전히 사라집니다.

TopDown Engine을 보유해서 애니메이션까지 쓰려면 `CATCOPS_TOPDOWNENGINE` 스크립팅
정의 심볼을 추가하세요. 레거시 프로토타입(`Assets/CatCops/`)의 연동 코드가
켜집니다. 정의가 없어도 컴파일에는 문제가 없습니다.
