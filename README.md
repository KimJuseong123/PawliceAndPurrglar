...
숫자키 명령을 기본 검증 경로로 유지하면서, 승인된 음성 수직 슬라이스에서는
`V`를 누르는 동안 최대 5초 녹음한 한국어 명령을 OpenAI로 분류합니다.
현재 실제 실행 명령은 경찰 강아지의 `TRACK`과 도둑 고양이의 `DISTRACT`
두 개뿐이며, 나머지는 아직 명령 후보입니다.
...
키보드와 음성 입력은 동일한 명령 ID와 실행 검증 경로를 사용합니다.

로컬 음성 중계 서버는 API 키를 Unity 프로젝트 밖의 환경 변수에서만 읽습니다.
Node.js 20 이상에서 다음처럼 실행합니다.
...
자세한 실행법과 비실시간 단위 테스트는
`Tools/VoiceGateway/README.md`를 확인합니다. API 키와 녹음·전사문은 저장소나
게임 로그에 기록하지 않습니다.
...
- 판매 NPC는 너구리 상인
- 숫자키 `1`의 경찰 `TRACK`, 숫자키 `2`의 도둑 `DISTRACT` 실행
- `V` Push-to-Talk와 로컬 OpenAI 중계 서버로 같은 명령 ID 분류
...
- 체포 방식과 세부 밸런스
- 최종 맵 크기와 건물 수
- 제출 빌드용 음성 중계 서버 배포 방식
- 나머지 동물 명령의 음성 분류와 효과
